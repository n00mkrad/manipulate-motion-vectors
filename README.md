...
>[!WARNING]
>Arbitrary scaling of motion vectors is possible and conceptually reasonable and is thus allowed by this plugin. However subsequent operations which use the motion vectors may not support all of the scaled properties. `ScaleVect` logs a warning when the snapped output geometry is known to be fully or partially incompatible with MVTools.

Scales `image_size`, `block_size`, `overlap`, `padding`, and the individual `motion_vector`s contained in `Analyse` output by arbitrary and independent `x` and `y` factors. Integer metadata is snapped to a nearby coherent grid which preserves the original block counts across the motion-vector pyramid when possible. This is mostly useful to use motion vectors generated on a downscaled clip to perform operations on the full size clip (e.g. calculating motion vectors at 1080p and applying them at 4K).

#### Usage
```py
manipmv.ScaleVect(vnode clip[, float scaleX=1.0, float scaleY=scaleX])
```
...
A clip generated by `mv.Analyse()` or which otherwise follows its conventions.
- scaleX:\
The scale factor to use horizontally. It must be finite and greater than zero.\
Values within `0.001` of a whole number are treated as that whole number for improved compatibility.\
Default value is `1.0` (no scaling).
- scaleY:\
The scale factor to use vertically. It follows the same validation and snapping rules as `scaleX`.\
Default value is the same as `scaleX`.

Scaled frame dimensions and padding are rounded to the nearest integer. Block size and overlap are selected from nearby integer values, preferring a grid which preserves the number of blocks at every pyramid level. If no nearby coherent grid exists, nearest-value rounding is used and a warning is logged. Motion-vector components are rounded to the nearest integer after scaling, while SAD values are scaled by the actual snapped block-area ratio.

MVTools only implements specific block pairs. For example, `10x10` cannot be generated by `Analyse` and has no Compensate/Degrain processing kernel, while `32x8` has processing kernels but cannot be generated by `Analyse`. `ScaleVect` still emits these and other serializable out-of-spec geometries, but reports all detected compatibility concerns in one VapourSynth warning.
...
```py
# Apply a fractional resize and allow ScaleVect to choose a nearby integer grid
SCALE_X = 1.5
SCALE_Y = 1.25
scaled_vectors = forward.manipmv.ScaleVect(SCALE_X, SCALE_Y)
```
